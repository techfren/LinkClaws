...
This operation updates SaaS administrative user configuration of the
...
The access type for the SaaS administrative user. If no access type is
specified, the READ_ONLY access type is used.
...
How long, in hours, the SaaS administrative user will stay enabled. If
no duration is specified, the default value 1 will be used.
...
Indicates if the SaaS administrative user is enabled for the
Autonomous AI Database.
...
A strong password for SaaS administrative user. The password must be a
minimum of nine (9) characters and contain a minimum of two (2)
uppercase, two (2) lowercase, two (2) numbers, and two (2) special
characters from _ (underscore), # (hashtag), or - (dash).
...
--time-saas-admin-user-enabled [datetime]

The date and time the SaaS administrative user was enabled at, for the
Autonomous AI Database.
