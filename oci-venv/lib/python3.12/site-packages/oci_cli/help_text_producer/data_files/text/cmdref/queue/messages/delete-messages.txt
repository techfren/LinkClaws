...
Deletes multiple messages from the queue or the consumer group. Only
messages from the same queue/consumer group can be deleted at once.
You must use the messages endpoint to delete messages. The messages
endpoint may be different for different queues. Use >>``<<GetQueue` <
https://docs.cloud.oracle.com/api/#/en/queue/latest/Queue/GetQueue/>`
__ to find the queue’s *messagesEndpoint*.
...
--entries [complex type]

The array of messages to delete from a queue. This is a complex type
