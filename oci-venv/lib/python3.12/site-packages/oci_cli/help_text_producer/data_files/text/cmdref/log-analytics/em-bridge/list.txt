...
A filter to return only log analytics enterprise manager bridge name
whose name matches the entire name given. The match is case-
insensitive.
...
Filter by the processing status of the latest upload from enterprise
manager.
...
A filter to return only log analytics enterprise manager bridges whose
lifecycleDetails contains the specified string.

--lifecycle-state [text]

A filter to return only log analytics enterprise manager bridges
...
The field to sort enterprise manager bridges by. Only one sort order
may be provided. Default order for timeCreated and timeUpdated is
descending. Default order for enterprise manager name is ascending. If
