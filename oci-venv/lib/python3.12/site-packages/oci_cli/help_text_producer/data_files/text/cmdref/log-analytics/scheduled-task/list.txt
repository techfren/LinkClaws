...
A filter to return only resources whose display name contains the
substring.
...
A filter to return only scheduled tasks whose stream action
savedSearchId matches the given ManagementSavedSearch id OCID exactly.
...
A filter to return only scheduled tasks whose stream action templateId
